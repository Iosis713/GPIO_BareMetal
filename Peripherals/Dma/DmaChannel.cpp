...
DmaChannel::DmaChannel(volatile DMA_Channel_TypeDef* const channel_, const DmaDirection direction)
...
void DmaChannel::Init(const DmaDirection dmaDir /*= DmaDirection::ReadFromPeripheralRX*/)
{
...
void DmaChannel::Configure(const uint32_t peripheralAddress, const uint32_t memoryAddress, const uint32_t length, const uint8_t dmaRequest /*RM 11.6.7 - 4 bit*/, const DmaMemoryPeripheralSize dmaSize /* = DmaMemoryPeripheralSize::bits_16*/)
{
	channel->CCR &= ~DMA_CCR_EN;
	...
	constexpr uint8_t cselrMask = 0xF;
